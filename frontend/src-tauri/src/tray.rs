...
fn build_menu<R: Runtime>(
    ...
    // Recording is always offered: models ship bundled, and the start path validates the
    // configured provider and reports any problem itself. (This used to be gated on
    // onboarding status, which no longer exists — leaving that gate in place would have
    // permanently disabled the tray's record item on new installs.)
    {
        match state {
